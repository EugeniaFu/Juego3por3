...
public class JuegoGato  extends JFrame implements ActionListener{
   ...
   JButton iniciar;
    ...
    public void verificar(){
        ...
        if(ganador==1){
           JOptionPane.showMessageDialog(this, "Felicidades "+ jugador1 + " ganÃ³" );
           bloquear();
        }
        if(ganador==2){
           JOptionPane.showMessageDialog(this, "Felicidades "+ jugador2 + " ganÃ³" );
           bloquear();
        }
    }
    ...
    public void limpiar(){//metodo para limpoiar el tablero por nosotros cada vez que se quiera inicar el juego
        ...
        for(int i=0; i<3; i++){
            for (int j=0; j<3; j++){
                tablero[i][j].setEnabled(true);//para q se vuelva a activar
